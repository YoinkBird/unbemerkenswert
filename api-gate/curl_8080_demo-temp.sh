...
resp_1="$(curl -s -X POST ${url}/notebooks -H 'Content-type:application/json' -d "${json_1[@]}")"
echo $resp_1
...
resp_2="$(curl -s -X PUT ${url}/notebooks/${id_1} -H 'Content-type:application/json' -d "${json_2[@]}")"
...
resp_2b="$(curl -s -X PUT ${url}/notebooks/${id_1} -H 'Content-type:application/json' -d "${json_2[@]}")"
...
if [[ "${_skip_deletion_test}" -eq 0 ]]; then
  ...
  resp_3="$(curl -s -X DELETE ${url}/notebooks/${id_1})"
